...
- [x] User-operated Republish reached `Ready`。Production passed the controlled first-message save、Gemini response、reload persistence、no-permission-error、`gemini-3.6-flash`，and Maps-load checks；the persistence remediation is complete。
- [x] Maps language remains browser-derived and independent from the app locale。No API-key restriction、Secret、region policy，or application code was weakened or changed to force English；use an English-browser profile for English demo capture。
